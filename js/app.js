(() => {
    ...
    const addActivity = (event) => {
        ...
        if (time.checkValidity() && date.checkValidity()) {
            ...
            totalMinutes = sumMinutes(totalMinutes, parseInt(workout.time));
            updateTotalTime();
        }
    };
    ...
    const deleteWorkout = (event) => {
        ...
        totalMinutes = subtractMinutes(totalMinutes, event.target.timeToDelete);
        updateTotalTime();
    };

    const updateTotalTime = () => {
        document.getElementById("sum").innerHTML = `${totalMinutes} minutes of exercise`;
    };
